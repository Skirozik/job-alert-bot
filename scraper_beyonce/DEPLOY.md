...
- **Volume and cost.** 12 runs/day. Cold start ~30 new jobs; steady state ~5-15. At ~$0.0046 per `classify` call (Haiku 4.5 at $1.00/$5.00 per 1M tokens, ~3,167-token system prompt), that is roughly **$0.28-$0.83/day, ~$8-25/month**, plus Modal compute of a few minutes/day.
- **Prompt caching is a silent no-op.** `classifier.py` sets `cache_control` on the system prompt, but the trimmed profile is 3,167 tokens against Haiku 4.5's 4,096-token minimum cacheable prefix, so the cache never engages. No penalty is charged either (no 1.25x write premium) — it is simply break-even versus omitting the directive. The parent pipeline's 16,972-char profile does clear the threshold. Editing the profile past ~4,096 tokens turns caching on silently.

...
9. **Membership in a group I am not in** — the posting states that only the company's own current/former/returning interns or employees, a pre-selected cohort, active-duty service members, or students at a specific school or region may apply. See "Eligibility restrictions the candidate cannot satisfy" below for the test and the confirmed live cases. This is a fact about the applicant pool, not a judgment about fit, so it is INELIGIBLE even when the role is a perfect match.
...
- **Eligibility restrictions the candidate cannot satisfy — a MEMBERSHIP test, run on every posting.** This is an addition to the exhaustive INELIGIBLE list above (it is rule 6 generalised past schools), and it applies even when the role is unambiguously software engineering, unambiguously a real internship, and a perfect stack match. **The test is not a list of strings to look up. It is one question: does any sentence anywhere in this posting say that only members of some group may apply — and is he outside that group?** If yes, INELIGIBLE, and this outranks every "when in doubt, choose APPLY_CAVEAT" default in this document, because membership in a group he cannot join is a hard fact, not a judgment call.
  - **Read the whole posting for it, not just the title.** The restriction is often in the title ("Current Interns Only- Technology Leadership Program", "2027 Returning Intern Software Engineer", "Intern Conversion: Software Developer", "Skillbridge Internship"), and a title-only check is cheap and worth running first. But the sentence is just as often a single declarative line in the body, sitting at the end of a block about something else entirely, phrased as a statement of fact rather than a requirement. **Confirmed live, and the reason this rule was rewritten:** RTX's "Software Engineer Intern - Spring 2027" (Melbourne FL) has a clean title and a perfect description — Python, CI/CD, Git, Linux, automation — and ends its Security Clearance block with "Security Clearance Status: Not Required This requisition is for an RTX intern returning for an internship in 2027." One sentence, no heading, no bullet, glued to the end of an unrelated block. It was tiered APPLY and pushed. Three sibling RTX postings carry the identical sentence and were all tiered APPLY_CAVEAT. Nothing about the role was wrong; the applicant pool was closed.
  - **The clearance block is not a safe place to stop reading.** The rules about "able to obtain" vs. "already hold" are about clearances only. Resolving a clearance line to "no issue" says nothing about the sentence next to it. Read to the end of the block.
  - **Shapes this takes (illustrations, not a lookup table).** Restricted to the company's own people: "This requisition is open to 2026 Truist Interns only", "For current/former HNTB Interns ONLY", "This job posting is intended for Williams' summer 2026 interns", "Must be a current Summer 2026 Baker Tilly Intern", "Successfully completed an internship with Blue Origin in 2026", "this role is only open to former Regions contract workers", any title containing "Intern Conversion" or "Returning Intern/Analyst" (a conversion req is by definition open only to that company's own interns), "internal candidates only". Restricted to a pre-selected cohort: "open only to Drexel University students who have already been officially selected for an interview", "for candidates who attended ICML '26 and met with a member of our team", Travelers' "Applications outside of this audience will not be considered". Restricted by military status: DoD SkillBridge and HOH/SpouseWorks fellowships (active duty, final 180 days), Oracle's OVIP veteran internship programme, "Internship - Active Duty Only". Restricted by where he studies: a school-specific co-op (rule 6), or "students currently enrolled at universities in Hong Kong or Singapore".
  - **The false positive to avoid, which is worse than the miss.** "Returning" and "return" are usually GOOD signals, and the absence of return-to-school language is a tell for a full-time role (see the full-time rules above). None of these restrict who may apply, and every one of them must stay surfaced: "Returning to a degree program after completion of the internship" (Google — already recorded here as a confirmed wrong SKIP), "the opportunity to earn a return internship or full-time offer" (Optiver), "may be invited to return as full-time Analysts" (Citi), "students returning to school after the internship" (AWS), "our intern conversion rate is over 70%", "we are an authorized DoD SkillBridge partner", "we also sponsor a Veterans Internship Program", and perks phrased for the intern class ("housing is provided for our summer interns only"). The discriminator is never the word — it is whether the sentence closes the applicant pool. A description that merely mentions a restricted programme it is not part of is an ordinary open posting.
  - He is a rising senior at Georgia State, a US citizen, and has never interned at any of these companies. He is not a current or former intern, employee or contractor there; he was not pre-selected; he is not on active duty; he does not attend a partner school. When the posting closes the pool against any of those, it is a hard INELIGIBLE, not APPLY_CAVEAT.
...
## GROUNDING RULE (applies to the `reason` field on every label)

**Name only what the posting actually says.** The `reason` is the one thing I
read before deciding where to spend an evening, so a reason that describes a job
other than the one linked is worse than no reason at all.

- Cite only technologies, tools, and responsibilities that appear **verbatim in
  the posting's own text**. Not inferred from the company, not inferred from the
  title, not "what a role like this usually involves."
- **Do not read my stack back to me as if the posting asked for it.** The failure
  looks like this: a posting that is four sentences of slogans naming no
  technology at all came back as "React/Next.js/Vite overlap." Nothing in it said
  React. That reason was generated by pattern-matching this profile onto the
  listing rather than reading the listing.
- If the posting names no specific technology, the honest reason is **"title-level
  match only"**. That is a perfectly good APPLY reason and I would rather have it
  than an invented one.
- Naming a technology the posting does not contain is a failure **even when the
  tier is correct**. I use the reason to prioritise, so a wrong reason wastes my
  time just as surely as a wrong tier.
- If the description looks like website navigation, a cookie banner, or a search
  widget rather than a job posting, say so in the reason instead of inventing a
  match from the title.
